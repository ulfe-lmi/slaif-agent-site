...
docker exec "${PROJECT}-browser-worker-1" sh -c \
  'test "$(find /ms-playwright -mindepth 1 -maxdepth 1 -type d | wc -l)" -eq 1; test -x /ms-playwright/chromium-1681091/chrome-linux64/chrome; test ! -e /usr/bin/npm; test ! -e /usr/bin/corepack; test "$(node --version)" = v24.18.1; /ms-playwright/chromium-1681091/chrome-linux64/chrome --version | grep -Eq "^Google Chrome for Testing 153[.]0[.]8010[.]52 *$"'
echo "browser-worker-image-policy: OK playwright=1.62.1 cft_revision=1681091 chromium=153.0.8010.52 browsers=chromium-only package-manager=absent"
