...
PUBLIC_AGENT_ACCEPTANCE_OUTPUT_FILE=
...
docker exec "${PROJECT}-browser-worker-1" sh -c \
  'test "$(find /ms-playwright -mindepth 1 -maxdepth 1 -type d | wc -l)" -eq 1; test -x /ms-playwright/chromium-1681091/chrome-linux64/chrome; test ! -e /usr/bin/npm; test ! -e /usr/bin/corepack; test "$(node --version)" = v24.18.1; /ms-playwright/chromium-1681091/chrome-linux64/chrome --version | grep -Eq "^Google Chrome for Testing 153[.]0[.]8010[.]36 *$"'
echo "browser-worker-image-policy: OK playwright=1.62.1 cft_revision=1681091 chromium=153.0.8010.36 browsers=chromium-only package-manager=absent"
...
python tools/compose/public_agent_acceptance.py \
  --project "$PROJECT" >"$PUBLIC_AGENT_ACCEPTANCE_OUTPUT_FILE"
cat "$PUBLIC_AGENT_ACCEPTANCE_OUTPUT_FILE"
...
docker exec "${PROJECT}-postgres-1" psql -U postgres -d slaif -Atc \
  "SET ROLE slaif_owner;
   SELECT count(*) = 7
      AND count(DISTINCT audit.operation_id) = 7
      ...
      AND count(*) FILTER (WHERE
          CASE
            ...
            WHEN audit.action LIKE 'PATCH /api/editor/v1/sites/%/theme'
              THEN 'theme-update'
            WHEN audit.action LIKE
              'PATCH /api/editor/v1/sites/%/pages/%/style'
              THEN 'page-style-update'
            ELSE 'unexpected'
          END = 'unexpected') = 0
      AND array_agg(
          CASE
      ...
      ) = ARRAY['page-create', 'theme-update', 'page-style-update',
                'page-style-update', 'component-add', 'component-add',
                'component-move']
...
docker exec "${PROJECT}-postgres-1" psql -U postgres -d slaif -Atc \
  "SET ROLE slaif_owner;
   SELECT count(*) = 8
      AND count(DISTINCT operation_id) = 8
...
echo "human-editor-envelope: OK workspace=HUMAN active audit=idempotent sequence=page-create,theme-update,page-style-update,page-style-update,component-add,component-add,component-move count=8"
