...
| §§15.4, 21.7–22.5, 24.6–24.7, 26–27, 30.6 | Bounded Agent component catalog/composition/design data plane with shared preview and browser evidence | External agent + human preview / REST/OpenAPI → Render/Web → browser worker | Objective 078-f on open [PR #77](https://github.com/ulfe-lmi/slaif-agent-site/pull/77) proves through public NGINX the immutable component catalog/composition slice and its browser evidence. Active 078-g extends the same PR with typed `design-system/v1`, component-local variant/layout/fixed-label responsive mutation, dynamic scope/resource checks, COW accounting, deterministic design races/cancellation, Puck round-trip, and Render/Web parity; its focused proof is not yet credited until all gates complete. Site-global theme, global-region, exact-workspace Puck, review, and publication remain outside this slice. | PARTIAL | Preserve 078-f; complete 078-g focused/remote evidence, then later rounds cover remaining theme/global behavior |
...
| 078 source revision | Public capability-bound catalog composition data plane plus the active 078-g bounded design-system/local variant-layout-responsive extension on open PR #77 | Remaining design/theme/global-region semantics, exact-workspace Puck, media references, MCP, freeze/review, promotion/publication, source reconstruction, cleanup, backup/restore, or contractual MVP completion |
