...
def downgrade() -> None:
    ...
    for function in ("content.slaif_redirect_page_target_dependency(uuid,text,uuid)",):
        ...
        op.execute(
            ...
            "slaif_agent_runtime, slaif_editor_runtime"
        )
    op.execute(
        "ALTER FUNCTION control.slaif_human_editor_workspace_assert("
        "uuid,uuid,uuid,uuid,text,boolean) OWNER TO slaif_owner"
    )
    op.execute(
        "REVOKE ALL ON FUNCTION control.slaif_human_editor_workspace_assert("
        "uuid,uuid,uuid,uuid,text,boolean) FROM PUBLIC"
    )
    op.execute(
        "GRANT EXECUTE ON FUNCTION control.slaif_human_editor_workspace_assert("
        "uuid,uuid,uuid,uuid,text,boolean) TO slaif_editor_runtime"
    )
