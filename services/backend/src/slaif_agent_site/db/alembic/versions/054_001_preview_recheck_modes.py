# ruff: noqa: E501
"""Separate short preview touch authorization from the COW recheck."""
...
revision: str = "054_001"
down_revision: str | Sequence[str] | None = "053_001"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_SIGNATURE = "text,bytea,uuid,uuid,integer,integer,integer"
_INTERNAL_SIGNATURE = _SIGNATURE + ",boolean"


def _secure(function: str, signature: str = _SIGNATURE) -> None:
    op.execute(f"ALTER FUNCTION control.{function}({signature}) OWNER TO slaif_owner")
    op.execute(
        f"REVOKE ALL ON FUNCTION control.{function}({signature}) "
        "FROM PUBLIC, slaif_preview_reader"
    )
...
def upgrade() -> None:
    op.execute(
        """
        CREATE FUNCTION control.slaif_render_preview_authorize_internal(
            p_public_id text, p_secret_digest bytea, p_workspace_id uuid,
            p_site_id uuid, p_idle_seconds integer, p_touch_interval_seconds integer,
            p_recent_auth_seconds integer, p_touch boolean
        ) RETURNS TABLE (
            user_account_id uuid, session_id uuid, workspace_id uuid, site_id uuid,
        ...
        BEGIN
            IF p_public_id IS NULL OR p_secret_digest IS NULL
               OR p_workspace_id IS NULL OR p_site_id IS NULL
               OR octet_length(p_secret_digest) IS DISTINCT FROM 32
               OR p_idle_seconds <= 0 OR p_touch_interval_seconds <= 0
               OR p_recent_auth_seconds <= 0
               OR p_public_id !~ '^sas2_[0-9a-f]{32}$' THEN RETURN; END IF;
            ...
            IF account_status IS DISTINCT FROM 'ACTIVE'
               ...
               OR selected_workspace.expires_at <= now_at THEN RETURN; END IF;
            ...
            IF NOT (
                EXISTS (SELECT 1 FROM control.platform_administrator AS administrator
            ...
            IF p_touch AND selected_session.last_seen_at
                    + make_interval(secs => p_touch_interval_seconds) <= now_at
            THEN
                UPDATE control.user_session AS session SET last_seen_at = now_at
    ...
    for wrapper, touch in (
        ("slaif_render_preview_authorize_touch", "true"),
        ("slaif_render_preview_authorize_recheck", "false"),
    ):
        op.execute(
            f"""
            CREATE FUNCTION control.{wrapper}(
                p_public_id text, p_secret_digest bytea, p_workspace_id uuid,
                p_site_id uuid, p_idle_seconds integer,
            ...
            SET search_path = pg_catalog ROWS 1 AS $fn$
                SELECT * FROM control.slaif_render_preview_authorize_internal(
                    p_public_id, p_secret_digest, p_workspace_id, p_site_id,
                    p_idle_seconds, p_touch_interval_seconds,
                    p_recent_auth_seconds, {touch}
                )
            $fn$
            """
        )
        _secure(wrapper)
        op.execute(
            f"GRANT EXECUTE ON FUNCTION control.{wrapper}({_SIGNATURE}) "
            "TO slaif_preview_reader"
        )
    _secure("slaif_render_preview_authorize_internal", _INTERNAL_SIGNATURE)
    op.execute(
        f"REVOKE ALL ON FUNCTION control.slaif_render_preview_authorize({_SIGNATURE}) "
        "FROM PUBLIC, slaif_preview_reader"
    )
...
def downgrade() -> None:
    for function in (
        ...
        "slaif_render_preview_authorize_internal",
    ):
        signature = _INTERNAL_SIGNATURE if function.endswith("internal") else _SIGNATURE
        op.execute(f"DROP FUNCTION IF EXISTS control.{function}({signature}) CASCADE")
    op.execute(
        f"GRANT EXECUTE ON FUNCTION control.slaif_render_preview_authorize({_SIGNATURE}) "
        "TO slaif_preview_reader"
    )
